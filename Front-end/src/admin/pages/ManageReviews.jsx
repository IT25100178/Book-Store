...
import { FiStar, FiCheck, FiX, FiMessageSquare, FiSearch, FiEyeOff } from 'react-icons/fi';
...
const ManageReviews = ({ showToast }) => {
    ...
    const approve = async r => {
        try {
            await api.updateReview(r.id, { ...r, status: 'Approved' });
    ...
    const hide = async r => {
        try {
            await api.updateReview(r.id, { ...r, status: 'Hidden' });
    ...
    const submitReply = async r => {
        ...
        try {
            await api.updateReview(r.id, { ...r, reply: replyText });
            setReplyId(null);
            setReplyText('');
                    ...
                    <div key={r.id} className="card" style={{ padding: 20, borderLeft: r.status === 'Pending' ? '4px solid #FACC15' : r.status === 'Hidden' ? '4px solid #EF4444' : '4px solid #4ADE80' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                            <div style={{ flex: 1 }}>
                                ...
                                {replyId === r.id && (
                                    <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
                                        ...
                                        <button className="btn btn-primary btn-sm" onClick={() => submitReply(r)}>Send</button>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
                                {r.status !== 'Approved' && (
                                    <button className="action-btn edit" title="Approve" onClick={() => approve(r)} style={{ color: '#4ADE80' }}><FiCheck /></button>
                                )}
                                ...
                                {r.status !== 'Hidden' && (
                                    <button className="action-btn" title="Hide" onClick={() => hide(r)} style={{ color: '#FACC15', border: '1px solid var(--border-color)' }}><FiEyeOff /></button>
                                )}
