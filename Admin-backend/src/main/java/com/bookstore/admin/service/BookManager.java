...
public class BookManager {
    ...
    public BookManager(JsonStorageUtil storage, ActivityLogManager activityLog, 
                       AuthorManager authorManager, PublisherManager publisherManager, 
    ...
    private void internalAdd(Book book) {
        ...
        if (book.getAuthorId() != null) {
            com.bookstore.admin.model.Author a = authorManager.getById(book.getAuthorId());
            if (a != null && !a.getBookIds().contains(book.getId())) {
                a.getBookIds().add(book.getId());
            }
        }
        
        // Sync Publisher
        if (book.getPublisherId() != null) {
            com.bookstore.admin.model.Publisher p = publisherManager.getById(book.getPublisherId());
            if (p != null && !p.getBookIds().contains(book.getId())) {
                p.getBookIds().add(book.getId());
                p.setBookCount(p.getBookIds().size());
            }
        }
        
        // Sync Genre
        if (book.getGenreId() != null) {
            com.bookstore.admin.model.Genre g = genreManager.getById(book.getGenreId());
            if (g != null && !g.getBookIds().contains(book.getId())) {
                g.getBookIds().add(book.getId());
            }
        }
    }
    ...
    private void internalRemove(Book book) {
        ...
        if (book.getAuthorId() != null) {
            com.bookstore.admin.model.Author a = authorManager.getById(book.getAuthorId());
            if (a != null) {
                a.getBookIds().remove(book.getId());
            }
        }
        ...
        if (book.getPublisherId() != null) {
            com.bookstore.admin.model.Publisher p = publisherManager.getById(book.getPublisherId());
            if (p != null) {
                p.getBookIds().remove(book.getId());
                p.setBookCount(p.getBookIds().size());
            }
        }
        ...
        if (book.getGenreId() != null) {
            com.bookstore.admin.model.Genre g = genreManager.getById(book.getGenreId());
            if (g != null) {
                g.getBookIds().remove(book.getId());
            }
        }
    }
    ...
    private void syncToTxt() {
        try {
            java.io.File file = new java.io.File("../Back-end/data/books.txt");
            ...
            try (java.io.PrintWriter writer = new java.io.PrintWriter(new java.io.FileWriter(file))) {
                for (Book b : bookMap.values()) {
                    String line = String.join("|",
                        ...
                        String.valueOf(b.getStockQuantity()),
                        String.valueOf(b.isNew()),
                        String.valueOf(b.isBestseller()),
                        String.valueOf(b.getPages()),
                        String.valueOf(b.getYear()),
                        b.getImage() != null ? b.getImage() : "📖"
                    );
                    writer.println(line);
                }
            }
        } catch (Exception e) {
            System.err.println("Failed to sync books to txt: " + e.getMessage());
        }
    }
}
